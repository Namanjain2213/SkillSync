      ...
      <aside class="sidebar">
        <div class="sidebar-brand">
          <div class="brand-logo">
            <app-icon name="shield" [size]="20" color="#fff"></app-icon>
          </div>
          <div>
  ...
  styles: [`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sidebar {
      ...
      background: linear-gradient(180deg, #1a0533 0%, #2d1b69 50%, #4a1080 100%);
    ...
    .brand-sub { font-size: 11px; color: #ce93d8; margin-top: 1px; }
    ...
    .user-role { font-size: 10px; color: #ce93d8; text-transform: uppercase; letter-spacing: 0.8px; margin-top: 2px; }

    .nav { flex: 1; padding: 6px 10px; display: flex; flex-direction: column; gap: 2px; }
    .nav-item {
      display: flex; align-items: center; gap: 11px;
      padding: 11px 14px; border-radius: 10px;
      color: #e1bee7; text-decoration: none;
      font-size: 13.5px; font-weight: 500; transition: all 0.15s;
    }
    .nav-item:hover { background: rgba(255,255,255,0.1); color: white; }
    .nav-item.active {
      background: rgba(171,71,188,0.25); color: #ce93d8; font-weight: 600;
    ...
    .main { flex: 1; display: flex; flex-direction: column; overflow: hidden; background: #f5f0ff; }
    .topbar {
      display: flex; align-items: center; justify-content: space-between;
      padding: 0 28px; height: 62px;
      background: white; box-shadow: 0 1px 10px rgba(0,0,0,0.07); flex-shrink: 0;
    }
    .page-title { font-size: 18px; font-weight: 700; color: #4a1080; }
    .welcome { font-size: 13px; color: #666; }
    .welcome strong { color: #4a1080; }
...
export class AdminShellComponent {
  ...
  navItems = [
    { label: 'Dashboard',       icon: 'home',       route: '/admin',         exact: true  },
    { label: 'All Users',       icon: 'users',      route: '/admin/users',   exact: false },
    { label: 'Create User',     icon: 'plus-circle',route: '/admin/create',  exact: false },
  ];

  constructor(private authService: AuthService, private router: Router) {}

  getInitial(): string { return (this.currentUser?.username?.[0] ?? 'A').toUpperCase(); }

  getPageTitle(): string {
    const url = this.router.url;
    if (url === '/admin') return 'Dashboard';
    if (url.includes('/users')) return 'User Management';
    if (url.includes('/create')) return 'Create User';
    return 'Admin Panel';
  }
